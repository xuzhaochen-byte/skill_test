#!/usr/bin/env python3
"""Resolve advertiser context from Aeolus Ads One Dataset by adv_id.

For an advertiser, query rows with non-empty external_url and select the row with
highest CVR (Conversions / Clicks). The selected row provides the customer
landing page URL, delivery country, and Aeolus primary/secondary industry for the
rest of the skill workflow.
"""
...
def build_body(*, adv_id: str, limit: int, min_clicks: int, min_impressions: int, last_sync_days: int) -> Dict[str, Any]:
    keys = [
        "country", "primary_industry", "secondary_industry", "external_url", "external_domain",
    ...
    # Advertiser IDs are 64-bit integers. Keep the filter value as a string in
    # the JSON body to avoid precision loss in JS/JSON layers for 19-digit IDs.
    # Aeolus accepts this for bigint dimensions and coerces it server-side.
    add_where(query_where, schema_where, field_id=FIELDS["advertiser_id"]["id"], name=FIELDS["advertiser_id"]["name"], op="in", val=[str(adv_id)], data_type="float")
    if min_clicks > 0:
        add_where(query_where, schema_where, field_id=FIELDS["clicks"]["originId"], name=FIELDS["clicks"]["name"], op="ge", val=[min_clicks], role_type=1, data_type="float")
    if min_impressions > 0:
        add_where(query_where, schema_where, field_id=FIELDS["impressions"]["originId"], name=FIELDS["impressions"]["name"], op="ge", val=[min_impressions], role_type=1, data_type="float")
    cvr_uid = next(x["uniqueId"] for x in dim_met_list if x["id"] == FIELDS["cvr"]["id"])
    sort = {"orderByList": [{"id": FIELDS["cvr"]["id"], "order": "desc", "uniqueId": cvr_uid}], "orderByListState": [{"id": FIELDS["cvr"]["id"], "order": "desc", "uniqueId": cvr_uid}], "type": "sort"}
...
def records_from_payload(payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for row in rows:
        ...
        if str(rec.get("External Website URL") or "").strip():
            records.append(rec)
    records.sort(key=lambda r: (-(parse_float(r.get("CVR (Clicks)")) or 0.0), -(parse_float(r.get("Conversions")) or 0.0), -(parse_float(r.get("Clicks (Destination)")) or 0.0)))
    for i, rec in enumerate(records, 1):
        rec["rank"] = i
    return records


def write_outputs(records: List[Dict[str, Any]], output_dir: Path, adv_id: str, source: Dict[str, Any], filters: Dict[str, Any]) -> Tuple[Path, Path, Path]:
    output_dir.mkdir(parents=True, exist_ok=True)
    selected = records[0] if records else None
    context = {"adv_id": adv_id, "selection_rule": "highest CVR (Clicks) among non-empty External Website URL rows", "selected": selected, "row_count": len(records), "source": source, "filters": filters, "rows": records}
    ...
    fields = [
        "rank", "Ad Country Code", "Primary Industry", "Secondary Industry", "External Website URL", "External URL Domains",
    ...
    if selected:
        ...
        env_path = output_dir / "adv_context.env"
        env_path.write_text(f"ADV_ID={adv_id}\n", encoding="utf-8")
    return json_path, csv_path, env_path


def main() -> int:
    parser = argparse.ArgumentParser(description="Resolve advertiser landing page/country/industry by adv_id from Aeolus.")
    ...
    body = build_body(adv_id=args.adv_id, limit=args.limit, min_clicks=args.min_clicks, min_impressions=args.min_impressions, last_sync_days=args.last_sync_days)
    body_path = args.output_dir / "aeolus_adv_context_body.json"
    ...
    filters = {"adv_id": args.adv_id, "p_date": f"lastSync {args.last_sync_days} day(s)", "min_clicks": args.min_clicks, "min_impressions": args.min_impressions, "external_url": "non-empty post-filter", "sort": "CVR (Clicks) desc"}
    source = {"app_id": APP_ID, "dataset_id": DATASET_ID, "region": REGION, "request_id": (payload.get("data") or {}).get("requestId")}
    json_path, csv_path, env_path = write_outputs(records, args.output_dir, args.adv_id, source, filters)
    if not records:
        raise SystemExit(f"No non-empty External Website URL rows found for adv_id={args.adv_id}")
    selected = records[0]
